...
APelotaDeRoca::APelotaDeRoca()
{
	...
	PelotaMaterial = LoadObject<UMaterial>(nullptr, TEXT("Material'/Game/StarterContent/Materials/M_CobbleStone_Pebble.M_CobbleStone_Pebble'"));
	Escala = FVector(0.5f, 0.5f, 0.5f);
}

void APelotaDeRoca::BeginPlay()
{
	Super::BeginPlay();
	CrearPelota();
}

void APelotaDeRoca::Tick(float DeltaTime)
{
...
void APelotaDeRoca::CrearPelota()
{
...
void APelotaDeRoca::RotarPelota()
{
	auto Rotacion = this->GetActorRotation();
	Rotacion += FRotator(5.0f, 0.0f, 5.0f);
